...
//read handler
static ssize_t proc_read(struct file *file, char _user *user_buf, size_t count, loff_t *ppos) {
...
// _exit marks this function module as module cleanup code.
// It is called when the module is unloaded. For code built into the kernel, exit code can be discarded because it cannot be unloaded .
